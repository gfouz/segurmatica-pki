...
async function getRequest() {
   ...
   if(id) { 
      try {
        const res = await axiosApi.get(`provincias/${id}`);
        const result = {
          data: res.data,
          status: res.status,
...
const onSubmit: SubmitHandler<IFormInput> = async (id) => {
   ...
   return (
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
     ...
     <StyledGetter>
      {status === "error" && <div><Spinner size="sm" />An error ocurred...</div>}
      {status === "loading" && <div><Spinner size="sm" />Loading...</div>}
      { data && ( 
        <div className='list-container'>
          <ul className= 'list'> 
           <li className='list__item'>Nro de ID: {data.id}</li>
           <li className='list__item'>Provincia: {data.name}</li>
           <li className='list__item'>Estado: {data.enabled.toString()}</li>
          </ul>
        </div>)
      }
    </StyledGetter>  
